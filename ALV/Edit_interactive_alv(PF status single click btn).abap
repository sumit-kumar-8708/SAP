...
* interactive_alv Report with PF Status( single click on First ALV list then again second ALV list Open and Update multiple values in desc)
...
FORM get_data_vbap  USING    p_rs_selfield_value.
  ...
  SELECT vbeln
   ...
   INTO TABLE gt_vbap
   FROM vbap
   WHERE vbeln = p_rs_selfield_value.
...
*     I_BUFFER_ACTIVE          = ' '
      ...
      i_callback_pf_status_set = 'SINGLE_CLICK_PF'
...
FORM single_click_pf USING rt_extab TYPE slis_t_extab.

  APPEND '%PC' TO rt_extab.

  SET PF-STATUS 'DISPLAY_CHILD_RECORD' EXCLUDING rt_extab. " Created in SE41 (copy of standard)
*      EXCLUDING rt_extab.
...
FORM user_command USING r_ucomm LIKE sy-ucomm
  ...
  CASE r_ucomm.
    ...
    WHEN '&IC1'.
      MESSAGE 'Single Click' TYPE 'I'.

    WHEN 'DISPLAY'.
*      MESSAGE 'Display Button' TYPE 'I'.

      gv_vbeln = rs_selfield-value. " store value gobally for redirect

      PERFORM conversion_input USING gv_vbeln.  " digit conversion 1 to 000000000001 for vbeln

      PERFORM get_data_vbap USING gv_vbeln.
